...
function InternetDetector:parseHosts()
	self.parsedHosts = {}
	local addr, port    = self:parseHost(self.serviceConfig.hosts)
	self.parsedHosts[1] = { addr = addr, port = port }
end
...
function InternetDetector:checkHosts()
	local checkFunc = (self.serviceConfig.check_type == 'icmp') and self.pingHost or self.TCPConnectionToHost
	local retCode   = 1
...
function InternetDetector:setServiceConfig(instance)
	if self:loadUCIConfig("host", instance) then
		self:parseHosts()
		if self.mode == 2 then
